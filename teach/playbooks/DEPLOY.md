...
2. Done when `curl -sI https://teach.zytact.com/<slug>/INDEX` returns 200 for the current vault. Give the user that URL. If curl cannot resolve the host right after the first deploy, the local resolver has cached the old miss. Check through `--resolve teach.zytact.com:443:$(dig +short teach.zytact.com @1.1.1.1 | head -n 1)` instead.
